...
RSpec.describe IapAuthenticator::Token do
	it 'should call google oauth api to generate the bearer token' do
		requestURI = IapAuthenticator::Token::TokenURI
		grant_type = IapAuthenticator::Token::JWTBearerType
		...
		response_jwt = IapAuthenticator::Token.generate_bearer_token( assertion )
	...
	it 'should call google oauth api and fail if assertion is invalid JWT signature' do
		...
		stub_request(:post, requestURI).
 		...
 		expect { IapAuthenticator::Token.generate_bearer_token( assertion ) }.to raise_error("Request failed with error: invalid_grant and description: Invalid JWT Signature.")
	end
end
