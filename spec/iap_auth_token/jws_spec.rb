...
RSpec.describe IapAuthToken::JWS do
	it 'should create a valid assertion' do
		...
		jws = IapAuthToken::JWS.new(private_key, refresh_time_seconds, client_email, client_id)
		assertion = jws.assertion
		decoded_token = JWT.decode assertion, public_key, true, { algorithm: IapAuthToken::JWS::ALGORITHM }
		expect(decoded_token[0]["iss"]).to eq(client_email)
		expect(decoded_token[0]["target_audience"]).to eq(client_id)
		expect(decoded_token[0]["aud"]).to eq(IapAuthToken::Token::TokenURI)
		expect(decoded_token[1]["alg"]).to eq(IapAuthToken::JWS::ALGORITHM)
	end

	it 'should handle JWS encoding failure' do
                jws = IapAuthToken::JWS.new( "", 3600, "", "")
